...
//template <intkgemm_4bit_inference_naive QUANT_TYPE, typename INP_TYPE, typename COMP_TYPE, typename OUT_TYPE>__global__ void kMatmul_inference_4bit(INP_TYPE *A, unsigned char *B, OUT_TYPE *out, int lda, int ldb, int rowsA, int colsA, int colsB);
...
template<typename T, int BLOCK_SIZE, int NUM_PER_TH> __global__ void kQuantizeBlockwiseInt4(float * code, T * __restrict__ const A, float *delta, float *min_val, unsigned char *out, const int n);
template<typename T, int BLOCK_SIZE, int THREADS, int NUM_PER_TH> __global__ void kDequantizeBlockwiseInt4(float *code, unsigned char * A, float *delta, float *min_val, T *out, const int blocksize, const int n);
...
template <typename T, int THREADS, int BITS> __global__ void kgemm_4bit_inference_naive(int M, int N, int K, T * __restrict__ const A, unsigned char *B,  float *delta, float *min_val, const float *datatype, T * out,  int lda, int ldb, int ldc, int blocksize);
