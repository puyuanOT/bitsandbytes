...
template <typename T> void quantizeBlockwiseInt4(float * code, T *A, float *delta, float *min_val, unsigned char *out, float* rand, int rand_offset, int blocksize, const int n)
{
  ...
  if(blocksize == 4096)
    ...
    kQuantizeBlockwiseInt4<T, 32, 2><<<num_blocks, 32>>>(code, A, delta, min_val, out, n);
...
template <typename T, int BITS> void gemm_4bit_inference_naive(int m, int n, int k, T * A,  unsigned char* B,  float *delta, float *min_val, float *datatype, T * out,  int lda, int ldb, int ldc, int blocksize)
{

	int num_blocks = (m+3)/4;

  kgemm_4bit_inference_naive<T, 128, BITS><<< num_blocks, 128, 0, 0 >>>(m,  n,  k, A,  B, delta, min_val, datatype, out, lda, ldb, ldc, blocksize);
  CUDA_CHECK_RETURN(cudaPeekAtLastError());
}
...
template void gemm_4bit_inference_naive<half, 16>(int m, int n, int k, half * A,  unsigned char* B,  float *delta, float *min_val, float *datatype, half * out,  int lda, int ldb, int ldc, int blocksize);
template void gemm_4bit_inference_naive<__nv_bfloat16, 16>(int m, int n, int k, __nv_bfloat16 * A,  unsigned char* B, float *delta, float *min_val, float *datatype, __nv_bfloat16 * out,  int lda, int ldb, int ldc, int blocksize);
template void gemm_4bit_inference_naive<float, 32>(int m, int n, int k, float * A,  unsigned char* B,  float *delta, float *min_val,  float *datatype, float * out,  int lda, int ldb, int ldc, int blocksize);
